...
export function initControlsUI({ renderGlobals, setState }) {
  const S = setState;
  ...
  let _lastUiNLayers = Math.max(
    1,
    Math.floor(renderGlobals.paramsState.nLayers || 1),
  );
  ...
  setSlabControlsEnabled(
    normRenderMode(renderGlobals.paramsState.renderMode) === "slab",
  );
